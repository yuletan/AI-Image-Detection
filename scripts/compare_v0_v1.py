...
def main() -> None:
    ...
    for r0 in base:
        ...
        o = v0[key]
        line = (f"{r0['transform'] + '=' + str(r0['param']):24s} "
                f"{o['auroc']:8.4f} ")
        tprs = f"{o['tpr_at_1fpr']:7.4f} "
        for c in cfgs:
            r = next(x for x in summ["variants"][c]
                     if (x["transform"], str(x["param"])) == key
                     and x["split"] == "test")
    ...
    for c in cfgs:
        for m in ("auroc", "tpr_at_1fpr", "acc", "ece"):
            vals = [r[m + "_mean"] for r in summ["variants"][c]
                    if r["split"] == "test"]
